...
public class SoundUtil {
    ...
    public void success(@NonNull Player user) {
        play(user.getUniqueId(), plugin.pConfig.getConfig().getString(ConfigManager.Path.SOUND_ACTION_SUCCESS.path, "ENTITY_VILLAGER_YES"));
    }

    public void fail(@NonNull Player user) {
        play(user.getUniqueId(), plugin.pConfig.getConfig().getString(ConfigManager.Path.SOUND_ACTION_FAILURE.path, "BLOCK_ANVIL_DESTROY"));
    }

    public void ding(@NonNull Player user) {
        play(user.getUniqueId(), plugin.pConfig.getConfig().getString(ConfigManager.Path.SOUND_ACTION_START.path, "ENTITY_ARROW_HIT_PLAYER"));
    }
    ...
    public void voteEvent(boolean isStart) {
        Bukkit.getOnlinePlayers().forEach(
                player -> play(player.getUniqueId(),
                        (isStart
                                ? plugin.pConfig.getConfig().getString(ConfigManager.Path.SOUND_VOTE_START.path, "ENTITY_ARROW_HIT_PLAYER")
                                : plugin.pConfig.getConfig().getString(ConfigManager.Path.SOUND_VOTE_END.path, "BLOCK_ANVIL_HIT")
                        )
                )
        );
    }
